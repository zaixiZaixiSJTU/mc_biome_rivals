...
foreach ($line in Get-Content -LiteralPath $sourcePath -Encoding UTF8) {
    ...
    $definitions.Add([ordered]@{
        id=$cardId; designId=$designId; contentVersion=15; collectible=(-not $isToken)
...
$definitionDocument = [ordered]@{ schemaVersion=3; contentVersion=15; source=$SourceMarkdown.Replace('\','/'); entries=$definitions }
