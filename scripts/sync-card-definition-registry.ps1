...
foreach ($line in Get-Content -LiteralPath $sourcePath -Encoding UTF8) {
    ...
    $definitions.Add([ordered]@{
        id=$cardId; designId=$designId; contentVersion=7; collectible=(-not $isToken)
...
$definitionDocument = [ordered]@{ schemaVersion=3; contentVersion=7; source=$SourceMarkdown.Replace('\','/'); entries=$definitions }
