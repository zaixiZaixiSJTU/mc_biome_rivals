...
foreach ($line in Get-Content -LiteralPath $sourcePath -Encoding UTF8) {
    ...
    $definitions.Add([ordered]@{
        id=$cardId; designId=$designId; contentVersion=6; collectible=(-not $isToken)
...
$definitionDocument = [ordered]@{ schemaVersion=3; contentVersion=6; source=$SourceMarkdown.Replace('\','/'); entries=$definitions }
