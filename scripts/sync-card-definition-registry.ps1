...
foreach ($line in Get-Content -LiteralPath $sourcePath -Encoding UTF8) {
    ...
    $definitions.Add([ordered]@{
        id=$cardId; designId=$designId; contentVersion=29; collectible=(-not $isToken)
...
$definitionDocument = [ordered]@{ schemaVersion=4; contentVersion=29; source=$SourceMarkdown.Replace('\','/'); entries=$definitions }
