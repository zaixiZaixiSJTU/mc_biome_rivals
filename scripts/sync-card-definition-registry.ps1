...
foreach ($line in Get-Content -LiteralPath $sourcePath -Encoding UTF8) {
    ...
    $definitions.Add([ordered]@{
        id=$cardId; designId=$designId; contentVersion=34; collectible=(-not $isToken)
...
$definitionDocument = [ordered]@{ schemaVersion=4; contentVersion=34; source=$SourceMarkdown.Replace('\','/'); entries=$definitions }
