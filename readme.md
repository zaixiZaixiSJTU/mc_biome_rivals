...
Nakama 权威规则已支持开局快照、部署校验、阶段切换、普通攻击、反击/死亡、英雄伤害、事件回放、结束回合与投降；Unity 侧已具备同构 DTO、快照/事件状态仓库和可替换传输接口。当前可玩场景默认使用离线规则执行器，安装并注册锁定版本的 Nakama Unity SDK 适配器后即可复用同一命令协议；嘲讽、冲锋、法术目标与具体卡牌效果仍是后续玩法迭代。
