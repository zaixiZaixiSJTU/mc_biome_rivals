...
- 雪屋在起始阶段按最大缺失生命自动治疗；并列时直接高亮 3D 场内单位供玩家点选，多个雪屋会在每次治疗后重新计算目标；
...
- `末影水晶`已接入结束阶段与亡语结算：3D 黑曜石基座上的悬浮晶核会显示末地紫蓄能脉冲，结束阶段伤害可被护甲吸收，被摧毁时则以真实伤害反噬拥有者；
- `岩浆怪`亡语已接入批量死亡队列：同批对象全部离场后召唤 1/1`小型岩浆怪`，优先复用来源释放的单位格；
- `流浪者`已接入“先选敌方战吼目标、再选己方部署格”的两阶段交互，施加不减攻击的`缓慢`；被敌方击杀时掉落`骨头`，战场使用 Minecraft 流浪者模型；
- 每回合开始抽牌、7 张手牌上限、公开爆牌、弃牌计数与空牌库递增疲劳；
- 权威联机开局按 GDD 执行先手 3 张、后手 4 张与双方各一次任意数量起手调度；两人确认前战场输入保持锁定；
- 法术、材料和装备的费用校验、实现状态提示与弃牌区流转；
- `熔岩献祭`、`腐肉`、`潜影壳`、`雪球`与`粉雪桶`已接入真实权威效果；粉雪桶使用 3D 地表高亮选择敌方单位，把可回放的`缓慢 1`强化至绑定 -2 攻击，目标控制者结束阶段恢复；其余效果在 UI 中明确标记为待接入；
...
`-WithMinecraftAssets` 会从本机已拥有的 Minecraft Java JAR 白名单提取 27 张方块贴图、15 张生物皮肤和卡牌图标。提取物与 Windows 构建均位于 Git 忽略目录，不会进入仓库。
