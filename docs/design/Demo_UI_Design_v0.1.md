...
- 空战场格默认不绘制任何框；合法部署时才在地表显示带间隙的青色发光像素网格，悬停合法格时网格变金、轻微抬升并扩大，已有单位的格子由 3D 模型占据。
...
槽位的屏幕空间矩形只负责射线命中，本身完全透明。`DemoSceneController` 将合法性和指针悬停状态传给 `DemoBattlefield3D`，由单个合并 Mesh 构成的世界空间地表网格负责透明度、发光、缩放和高度动画；无选牌、无悬停或槽位已占用时网格不渲染。规则状态仍只存在于 `DemoLocalMatch`，不会被动画反向修改。
