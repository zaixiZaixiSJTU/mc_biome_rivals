...
- 56 张可收集牌。
- 18 张不可收集衍生物。
- 69 张有规则文本的牌预留 `effect.<cardId>.01`；其中 50 张为 `IMPLEMENTED`，其余 19 张为 `PENDING`。
- 5 张无规则文本衍生物状态为 `NONE`。
- 卡牌定义 Schema v4 / 内容版本 v38 注册通用 `keywords`、二元支付配方、可选定向战吼、SI-004 山羊越位移动与 SI-007 雪屋起始阶段治疗，以及既有抽牌、召唤、状态、考古、光环和临时属性效果。当前注册 4 张 `TAUNT`，并预留 `CHARGE`。雪屋链路覆盖“起始抽牌 → 最大缺失生命判定 → 唯一目标自动治疗 / 并列场内选择 → 多来源重算”。
