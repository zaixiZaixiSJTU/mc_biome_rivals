...
- 56 张可收集牌。
- 18 张不可收集衍生物。
- 69 张有规则文本的牌预留 `effect.<cardId>.01`；其中 51 张为 `IMPLEMENTED`，其余 18 张为 `PENDING`。
- 5 张无规则文本衍生物状态为 `NONE`。
- 卡牌定义 Schema v4 / 内容版本 v39 注册通用 `keywords`、二元支付配方、可选定向战吼、SI-004 山羊越位移动、SI-007 雪屋起始阶段治疗与 ED-007 末影水晶结束阶段/亡语伤害，以及既有抽牌、召唤、状态、考古、光环和临时属性效果。当前注册 4 张 `TAUNT`，并预留 `CHARGE`。末影水晶链路覆盖普通伤害护甲吸收、稳定多来源顺序、真实伤害反噬和致命立即终止。
