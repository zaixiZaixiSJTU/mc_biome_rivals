...
- 56 张可收集牌。
- 18 张不可收集衍生物。
- 69 张有规则文本的牌预留 `effect.<cardId>.01`；其中 47 张为 `IMPLEMENTED`，其余 22 张为 `PENDING`。
- 5 张无规则文本衍生物状态为 `NONE`。
- 卡牌定义 Schema v4 / 内容版本 v35 注册通用 `keywords`、二元支付配方、CD-001 牌库顶窥视、CD-002 普通战斗伤害施毒、CD-004 第二张牌感测、CD-006 黑暗与抽牌、CD-007 单牌回合圆石产出、CD-008 空位卫道士新兵召唤、DB-003 考古选择、DB-005 本回合出土降费、DB-007 双格神殿与掩埋链、SI-002 雪球生成战吼、TK-002 小麦定向治疗与动物临时增攻、CD-005 建筑条件临时增攻，以及 `manualPlayAllowed` 主动打出权限。当前注册 4 张 `TAUNT`，并预留 `CHARGE`。深暗教学链路同时覆盖第二张牌触发幽匿感测体 → 黑暗限制首次敌方战场对象只能选择当前合法行的左右边缘 → 指定后解除本回合限制，以及 `洞穴蜘蛛造成攻击/反击伤害 → 中毒 3 → 控制者结束阶段毒伤`。
