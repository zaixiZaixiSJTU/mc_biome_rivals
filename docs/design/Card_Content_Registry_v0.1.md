...
- 56 张可收集牌。
- 18 张不可收集衍生物。
- 69 张有规则文本的牌预留 `effect.<cardId>.01`；其中 49 张为 `IMPLEMENTED`，其余 20 张为 `PENDING`。
- 5 张无规则文本衍生物状态为 `NONE`。
- 卡牌定义 Schema v4 / 内容版本 v37 注册通用 `keywords`、二元支付配方、可选定向战吼、SI-004 山羊越位移动，以及既有抽牌、召唤、状态、考古、光环和临时属性效果。当前注册 4 张 `TAUNT`，并预留 `CHARGE`。山羊链路覆盖“选择友军 → 选择中间部署格 → 确定性跨格移动 → 海豚/守卫者反应 → 山羊本回合增攻”，且允许无目标普通部署。
