...
卡牌目标选择由稳定 `effectId` 规则注册，不再写死为敌方单位：当前可区分敌方生物、己方生物与己方建筑/结构，并以贴地高亮只标记合法目标。离线规则镜像与 Nakama 权威规则共同支持 9 个已注册效果，其余 `PENDING` 效果仍会在扣费前拒绝。首个亡语 `潜影贝`会生成`潜影壳`；`CARD_GENERATED` 在己方手牌中显示真实卡牌，在敌方手牌中只增加未知占位，满手转弃牌时双方都能看到卡牌身份。
